...
def chat(chat_id):
  ...
  for i in range(len(messages)):
    messages[i].update(
      {"sender_email": getClientById(messages[i]["sender_id"])["email"]})
    print('chat', messages)
...
#Добавление в итоговый список email пользователя (можно отображать просто id)
  for i in range(len(chats)):
    ...
    if chats[i]["first_client_id"] == client_id:
      chats[i].update(
        {"chat_with": getClientById(chats[i]["second_client_id"])["email"]})
    elif chats[i]["second_client_id"] == client_id:
      chats[i].update(
        {"chat_with": getClientById(chats[i]["first_client_id"])["email"]})
  ...
  if request.method == "POST":
    ...
    if not user_from_db:
      return render_template(
        "login.html", error="Пользователь с такой почтой и паролем не найден")
    else:
      if email == user_from_db['email'] and password == user_from_db[
          'password']:
